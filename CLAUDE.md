...
- `setup.sh` — Основной интерактивный инсталлятор (~580 строк). Определение системы, установка пакетов, деплой конфигов, установка плагинов, настройка разрешений.
...
3. **Выбор компонентов** — все промпты `[Y/n]` с дефолтом "да", кроме расширенных разрешений `[y/N]`
...
- Расширенные разрешения (`[y/N]`) — создаёт `~/.claude/settings.json` с полным списком разрешённых инструментов (Bash, Edit, Write, Read, Glob, Grep, WebFetch, WebSearch, NotebookEdit, Task)
...
| `np <имя>` | `mkdir -p <dir>/<имя>` + `tmux new-window -n <имя>` |
